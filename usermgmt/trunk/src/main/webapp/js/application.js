...
$(document).ready(function() {
	...
	var userTable = $("#user-table").DataTable( {
	    ...
	    "aoColumns": [
	        ...
	        {     // fifth column (Edit link)
	            ...
	            "fnRender": function (oObj)                              
	            {
	                ...
	                return "<a href='/users/edit?id=" + oObj.aData[0] + "'>Edit</a>";
	            }
	        },
	    ...
	    "columnDefs": [ {
	        "targets": -1,
	        "data": null,
	        "defaultContent": "<button class='edit-entity'>Edit</button> <button class='delete-entity'>Delete</button>"
	    } ]
	});

	$('#user-table tbody').on( 'click', 'button.edit-entity', function () {
	    var data = userTable.row( $(this).closest('tr') ).data();
	    var userId = data[ 'userId' ];
	    location.href = '/editUser?id=' + userId;
	});
	$('#user-table tbody').on( 'click', 'button.delete-entity', function () {
	    var data = userTable.row( $(this).closest('tr') ).data();
	    var userId = data[ 'userId' ];
	    location.href = '/deleteUser?id=' + userId;
	});

$(".delete-btn").click(function(e) {
	id = $(this).attr('id').split('-delete')[0];
	$("#deleteId").val(id);
	$('#modal-message').html("Are you sure you want to delete?");
	$("#delete-yes").show();
	$("#delete-no").text("No");
...
$("#delete-yes").click(function(e) {
	 $.ajax({
         url : $("#deleteURL").val()+"?id="+$("#deleteId").val(),
         data : '',//JSON.stringify($('#userSignUpForm').serializeArray()),
         type : 'GET',
         dataType: "json",
...
$("#addFavIp").click(function (e) {
	e.stopPropagation();
	e.preventDefault();
  $("#favIps").append('<div><input type="text" name="favouriteIps[]" data-rule-ipv4="true"><button  class="deleteFavIp btn">Delete</button></div>');
});
$("body").on("click", ".deleteFavIp", function (e) {
	e.stopPropagation();
	e.preventDefault();
