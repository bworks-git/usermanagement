	...
	<div class="container-fluid" id="content">
					...
					<div class="">
						<ul class="stats">
							<form action="addRole" method="GET">
								<button type="submit" class="btn btn--icon"><i class="fa fa-plus-square"></i>Create Role</button>
							</form>
						</ul>
					</div>
				</div>
						...
						<div class="box box-color box-bordered">
							...
							<div class="box-content nopadding">
								<table
									...
									data-column_filter_types="text,text,text,select,select,select,select,daterange,daterange,null"
									data-column_filter_dateformat="yyyy-MM-dd HH:mm:ss"
									id="role-table">
									<thead>
										<tr>
											<!--
									...
									<c:forEach var="role" begin="0" end="${fn:length(roles)-1}" items="${roles}">
										<tr>
											...
											<td>
											<c:if test="${not empty role.permissionSet}">
											<c:forEach var="permission" begin="0" end="${fn:length(role.permissionSet)-1}" items="${role.permissionSet}">
											...
											<td>
											<c:set var="createdDate" value="${role.create_date}" />
											<fmt:formatDate type="both" pattern="yyyy-MM-dd HH:mm:ss" value="${createdDate.time}" /> 
											</td>
											<td>
											<c:set var="modifiedDate" value="${role.modified_date}" />
											...
											<td class='hidden-480'>
												...
												<a href="editRole?id=${role.id}" class="btn" rel="tooltip" title="Edit"> <i class="fa fa-edit"></i></a> 
												<a href="#" id="${role.id}-delete" class="delete-btn btn" rel="tooltip" title="Delete"> <i class="fa fa-times"></i></a>
											</td>
										</tr>
									</c:forEach>
									</c:if>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<input type="hidden" id="deleteURL" value="deleteRole"/>
				<!-- /.modal-body -->
